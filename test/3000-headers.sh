...
test_run 'test the scoreHeader parameter' '
	cat <<-EOD | "$FILTER_BIN" $FILTER_OPTS -scoreHeader $FILTER_DOMAINS | sed "0,/^register|ready/d" >actual &&
	config|ready
	...
	filter-dataline|7641df9771b4ed00|1ef1c203cc576e5d|X-DNSBL-Score: 42
