...
test_run 'test the connect filter with a non-reputable IP address' '
	...
	filter|0.5|0|smtp-in|connect|7641df9771b4ed00|1ef1c203cc576e5d||pass|1.2.3.60:33174|1.1.1.1:25
	EOD
	cat <<-EOD >expected &&
...
test_run 'test block phase: connect' '
	cat <<-EOD | "$FILTER_BIN" $FILTER_OPTS -blockAbove 50 -blockPhase connect $FILTER_DOMAINS | sed "0,/^register|ready/d" >actual &&
	config|ready
	report|0.5|0|smtp-in|link-connect|7641df9771b4ed00||pass|1.2.3.60:33174|1.1.1.1:25
	filter|0.5|0|smtp-in|connect|7641df9771b4ed00|1ef1c203cc576e5d||pass|1.2.3.60:33174|1.1.1.1:25
	EOD
...
test_run 'test block phase: helo' '
	cat <<-EOD | "$FILTER_BIN" $FILTER_OPTS -blockAbove 50 -blockPhase helo $FILTER_DOMAINS | sed "0,/^register|ready/d" >actual &&
	config|ready
	report|0.5|0|smtp-in|link-connect|7641df9771b4ed00||pass|1.2.3.60:33174|1.1.1.1:25
	filter|0.5|0|smtp-in|connect|7641df9771b4ed00|1ef1c203cc576e5d||pass|1.2.3.60:33174|1.1.1.1:25
...
test_run 'test block phase: ehlo' '
	cat <<-EOD | "$FILTER_BIN" $FILTER_OPTS -blockAbove 50 -blockPhase ehlo $FILTER_DOMAINS | sed "0,/^register|ready/d" >actual &&
	config|ready
	report|0.5|0|smtp-in|link-connect|7641df9771b4ed00||pass|1.2.3.60:33174|1.1.1.1:25
	filter|0.5|0|smtp-in|connect|7641df9771b4ed00|1ef1c203cc576e5d||pass|1.2.3.60:33174|1.1.1.1:25
...
test_run 'test block phase: mail-from' '
	cat <<-EOD | "$FILTER_BIN" $FILTER_OPTS -blockAbove 50 -blockPhase mail-from $FILTER_DOMAINS | sed "0,/^register|ready/d" >actual &&
	config|ready
	report|0.5|0|smtp-in|link-connect|7641df9771b4ed00||pass|1.2.3.60:33174|1.1.1.1:25
	filter|0.5|0|smtp-in|connect|7641df9771b4ed00|1ef1c203cc576e5d||pass|1.2.3.60:33174|1.1.1.1:25
...
test_run 'test block phase: rcpt-to' '
	cat <<-EOD | "$FILTER_BIN" $FILTER_OPTS -blockAbove 50 -blockPhase rcpt-to $FILTER_DOMAINS | sed "0,/^register|ready/d" >actual &&
	config|ready
	report|0.5|0|smtp-in|link-connect|7641df9771b4ed00||pass|1.2.3.60:33174|1.1.1.1:25
	filter|0.5|0|smtp-in|connect|7641df9771b4ed00|1ef1c203cc576e5d||pass|1.2.3.60:33174|1.1.1.1:25
...
test_run 'test with invalid block phase: data-line' '
	cat <<-EOD | "$FILTER_BIN" $FILTER_OPTS -blockAbove 50 -blockPhase data-line $FILTER_DOMAINS; [ "$?" -eq 1 ]
	config|ready
	report|0.5|0|smtp-in|link-connect|7641df9771b4ed00||pass|1.2.3.60:33174|1.1.1.1:25
	filter|0.5|0|smtp-in|connect|7641df9771b4ed00|1ef1c203cc576e5d||pass|1.2.3.60:33174|1.1.1.1:25
	EOD
